...
public class DoublyLinkedList<E> extends GeneralList<E> implements List<E>,Iterable<E>,Queue<E>{
    ...
    public void removeNNodes(E data, int n){
       Node<E> tmp = searchNode(data);
       if(tmp != null)
           removeNNodes(tmp.getNext(),n,1);
    }
    
    private void removeNNodes(Node<E> p,int n,int cont){
        if(cont<=n){
            if(p != null){
                if(p == lastD){
                    Node<E> tmp = p.getPrevious();
                    p.setPrevious(null);
                    tmp.setNext(null);
                    p.setData(null);
                    lastD = tmp;
                    efectivo--;
                    removeNNodes(null,n,++cont);
                } else {
                    Node<E> tmp2 = p.getNext();
                    Node<E> tmp3 = p.getPrevious();
    ...
    public void removeAll(E element){
        if(!(isEmpty() || element == null)){            
            removeAll(firstD,lastD,element,0,efectivo-1);
        }
        
    }
    
    private void removeAll(Node<E> p,Node<E>q,E element,int i,int j){
        if(i<=j){
            if(i == j){
                if(p.getData().equals(element)){
                    Node<E> tmp = p.getPrevious();
                        tmp.setNext(p.getNext());
                        p.getNext().setPrevious(tmp);
                        p.setData(null);
                        p.setNext(null);
                        p.setPrevious(null);
                        efectivo--;
                ...
                if (p != firstD && q != lastD) {
                    ...
                    if (q.getData().equals(element)) {
                        Node<E> tmp = q.getPrevious();
                        tmp.setNext(q.getNext());
                        q.getNext().setPrevious(tmp);
                        q.setData(null);
                        q.setNext(null);
                        q.setPrevious(null);
                        efectivo--;
                    ...
                    if (q.getData().equals(element)) {
                        Node<E> temp = lastD.getPrevious();
                        lastD.setData(null);
                        lastD.setPrevious(null);
                        temp.setNext(null);
                        lastD = temp;
                        efectivo--;
                        j = efectivo - 1;
    ...
    public DoublyLinkedList<E> split(int origin,int destiny){
        DoublyLinkedList<E> result = new DoublyLinkedList<>();
        if((origin >= 0 && origin < efectivo) && (destiny >= 0 && destiny < efectivo) && (origin < destiny)){
            int cont = 0;
            for(Node<E> v = firstD; v != null; v = v.getNext()){
                if(cont >= origin){
                    if(cont<destiny)
